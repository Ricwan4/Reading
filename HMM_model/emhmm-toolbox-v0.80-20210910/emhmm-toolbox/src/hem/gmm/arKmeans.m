function [centers,mincenter,mindist,q2,quality] = arKmeans(data,initcenters,method,stdDim)
% Computes K-mean (initalized with subset furthest first 
%
...
% stdDim = 1 standardize features before clustering, 0 (default) no scaling

tic
if nargin < 3 method = 2; end
if nargin < 4 stdDim = 0; end

[n,dim] = size(data);

%store original data to return, and standardize data if needbe
origData = data;
if stdDim
...
if max(size(initcenters)) == 1
    k = initcenters;
    %pick subset and subset size
    ridx = randperm(n);
    ssSize = min([ceil(2*initcenters*log(initcenters)), n]);
    %subset furthest first
    [centers, mincenter, mindist,lower,computed] = arAnchors(1,k,data(ridx(1:ssSize),:));    
    %total = computed;
    %skipestep = 1;
...
%project data back into original space if it had been analyzed in
%standardized space.
if stdDim
    centers = centers.*repmat(std(origData),k,1) + repmat(mean(origData),k,1);
end
