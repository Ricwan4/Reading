...
if isfield(hmm, 'stats')
  if ~usegroups
    hmm.stats.emit_vcounts = hmm_old.stats.emit_vcounts(cl);
    if isfield(hmm.stats, 'emit')
      hmm.stats.emit         = hmm_old.stats.emit(cl);
    end
    
    % check if anything left
    hfields = {'emit_vcounts', 'emit'};
    fn = fieldnames(hmm.stats);
    for ff=1:length(fn)
      if ~any(strcmp(fn{ff}, hfields))
        error(sprintf('%s not handled', fn{ff}));
      end
    end
  else
